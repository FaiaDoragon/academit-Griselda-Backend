...
export class MainArticleController {
  ...
  @ApiOperation({ summary: 'Crear un nuevo artículo principal', description: 'Crea un nuevo artículo principal con los datos proporcionados y opcionalmente una imagen.' })
  @ApiCreatedResponse({ description: 'El artículo principal se ha creado exitosamente.' })
  ...
  @Get()
  @ApiOperation({ summary: 'Obtener todos los artículos principales', description: 'Recupera una lista de todos los artículos principales existentes.' })
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Obtener un artículo principal por ID', description: 'Recupera un artículo principal específico usando su ID.' })
  @ApiOkResponse({ description: 'El artículo principal se ha recuperado exitosamente.' })
  ...
  @Patch(':id')
  @UseInterceptors(FileInterceptor('image'))
  @ApiOperation({ summary: 'Actualizar un artículo principal', description: 'Actualiza los datos de un artículo principal existente y opcionalmente su imagen.' })
  @ApiOkResponse({ description: 'El artículo principal se ha actualizado exitosamente.' })
  @ApiNotFoundResponse({ description: 'Artículo principal no encontrado.' })
  @ApiBadRequestResponse({ description: 'Solicitud incorrecta. Por favor, revisa tus datos de entrada.' })
  @ApiInternalServerErrorResponse({ description: 'Error interno del servidor. Por favor, intenta nuevamente más tarde.' })
  @ApiConsumes('multipart/form-data')
  ...
  update(@Param('id') id: number, @Body() updateMainArticleDto: UpdateMainArticleDto, @UploadedFile() file: any) {
    return this.mainArticleService.update(id, updateMainArticleDto, file);
  }

  @Delete(':id')
  @ApiOperation({ summary: 'Eliminar un artículo principal', description: 'Elimina un artículo principal existente usando su ID.' })
  @ApiNoContentResponse({ description: 'El artículo principal se ha eliminado exitosamente.' })
  @ApiNotFoundResponse({ description: 'Artículo principal no encontrado.' })
  @ApiBadRequestResponse({ description: 'Solicitud incorrecta.' })
